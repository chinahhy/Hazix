...
- **电视首页改为完整首屏**：轮播介绍与「最近热播」竖版海报占据首屏，海报卡片贴近屏幕底部；
  「最近观看」移到首屏下方，不再与海报同时挤在首页开场画面。
- **遥控器下键直达观看记录**：焦点在海报卡片上时，按下键会滚动到「最近观看」并聚焦首条记录；
  没有历史记录时也会显示可聚焦的空状态提示。
- 网页预览同步采用相同的首屏层次与方向键导航；手机版安装包随电视端共用版本号，本次没有更改手机原生界面。

## 3.7.4 — 2026-09-22

- **按 TCL 真机坐标再次下移首页海报**：实测电视为 1920×1080、240 dpi，v3.7.3 的首张海报
  位于 `y=491..798px`，仍然偏高。本版保持标题、简介和播放按钮不动，将「最近热播」与整排卡片
  再下移 64dp（该电视上为 96px），首张海报预期落在 `y=587..894px`，与上方操作区彻底拉开。

## 3.7.3 — 2026-09-22

- **修复首页海报遮挡上方内容**：上一版放大了海报卡并提高聚焦缩放，但没有同步增加
  首页巨幕的垂直空间，导致「最近热播」一行向上侵入简介和操作按钮区。现在保持上方文案不动，
  将海报标题与整排卡片下移 40dp，并为 1.10 倍聚焦外扩留出安全间距。

## 3.7.2 — 2026-09-22

- **首页现在打开即进入内容**：电视端首次进入首页会把焦点直接放到第一张节目卡，不再先停在
  「播放」按钮；从详情页返回时仍恢复此前选中的卡片与滚动位置。
- **预览从节目中段开始**：焦点稳定 850ms 后加载对应节目，播放器拿到有效时长并进入 Ready
  状态后先跳到 50% 位置，再静音循环播放；短片与未知时长直播流保持从头播放，避免错误 seek。
- **首页图片与卡片清晰度提升**：优先使用 hdao.tv 首页已经筛选好的 `hero` 队列和 TMDB 原图，
  低清兜底内容后置；海报卡扩大到 132×186dp、聚焦缩放提高到 1.10，并启用高质量图像过滤。
- **浏览器预览同步更新**：首卡自动聚焦，方向键切卡时标题、背景与预览视频保持一致；焦点停在
  卡片上时暂停 8 秒自动轮播，修复标题与当前预览节目错位的问题。手机布局仍不自动下载视频。
