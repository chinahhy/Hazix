...
- 分类、首页与搜索三处的返回状态都会保留：滚动位置不丢，光标回到刚才打开的那张卡片，
  搜索词与结果也不会被清空；
- 「最近观看」按节目 ID 与标准化片名双重去重；
- 详情页可一键收藏，「我的收藏」显示在首页；搜索页记住最近 8 条关键词，可一键复用；
- 遥控器 D-pad 显式焦点导航与高亮；
- ExoPlayer 原生 HLS 播放，支持字幕轨与音轨切换、0.5×–2.0× 倍速；
- 基于 GitHub Release 的应用内更新，下载后校验来源、大小、SHA-256、应用 ID、版本号与签名证书。
...
| 按键 | 浏览界面 | 播放器 |
| --- | --- | --- |
| 方向键 | 移动焦点／滚动列表 | 左右 ±10 秒；上下切换集数；播放设置打开时改为上下选行、左右调整 |
| OK / Enter | 打开聚焦项 | 播放／暂停；**长按**打开播放设置 |
| 菜单／设置键 | — | 打开或关闭播放设置（字幕 · 音轨 · 倍速） |
| 返回 | 返回上一页 | 先关闭播放设置；再按一次保存进度并退出播放 |
